...
const AdminEditTheatre = () => {
    ...
    const fetchTheatre = async () => {
        try {
            ...
            if (res.data.success) {
                const t = res.data.theatre;
                setForm({
                    theatreName: t.theatreName,
                    city: t.city,
                    location: t.location
                });
                setSelected(t.movies.map(m => m._id));
            }
        } catch (err) {
            console.error(err);
            toast.error("Failed to load theatre");
        }
    };
    ...
    const update = async (e) => {
        e.preventDefault();
        try {
            const res = await API.put(`/theatres/${id}`, {
                ...form,
                movies: selected
            });
                ...
                <div>
                    ...
                    <div className="grid grid-cols-2 gap-2 max-h-64 overflow-auto p-2 bg-gray-900 rounded">
                        {movies.map(m => (
                            <label
                                key={m._id}
                                className={`p-2 rounded border ${selected.includes(m._id) ? 'border-red-500' : 'border-transparent'} cursor-pointer flex items-center gap-2`}
                            >
                                <input
                                    type="checkbox"
